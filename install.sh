...
install_x-ui() {
    ...
    wget --inet4-only -O /usr/bin/x-ui-temp https://raw.githubusercontent.com/serogaq/3x-ui/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        wget --inet4-only -O /etc/init.d/x-ui https://raw.githubusercontent.com/serogaq/3x-ui/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
