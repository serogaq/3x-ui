...
update_x-ui() {
	...
	${wget_bin} -O /usr/bin/x-ui https://raw.githubusercontent.com/serogaq/3x-ui/main/x-ui.sh >/dev/null 2>&1
	if [[ $? -ne 0 ]]; then
		echo -e "${yellow}Trying to fetch x-ui with IPv4...${plain}"
		${wget_bin} --inet4-only -O /usr/bin/x-ui https://raw.githubusercontent.com/serogaq/3x-ui/main/x-ui.sh >/dev/null 2>&1
		if [[ $? -ne 0 ]]; then
	...
	if [[ $release == "alpine" ]]; then
		...
		${wget_bin} -O /etc/init.d/x-ui https://raw.githubusercontent.com/serogaq/3x-ui/main/x-ui.rc >/dev/null 2>&1
		if [[ $? -ne 0 ]]; then
			${wget_bin} --inet4-only -O /etc/init.d/x-ui https://raw.githubusercontent.com/serogaq/3x-ui/main/x-ui.rc >/dev/null 2>&1
			if [[ $? -ne 0 ]]; then
